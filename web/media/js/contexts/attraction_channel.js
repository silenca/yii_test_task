 ...
 */
var currentPage = "attraction-channel";
var dataTable,
...
$(function () {
    ...
    var initTable = function () {
        var table = $('#attraction-channel-table');
        ...
        var settings = {
            ...
            "order": [],
            "ajax": {
                url: "/attraction-channel/get-data", // json datasource
    ...
    if ($('#attraction-channel-table').length) {
        initTable();
        ...
        $('#column_filter_apply').on('click', function () {
            ...
            $.get('/attraction-channel/hide-columns', {hide_columns: hide_columns}, function () {
                location.reload();
            });
            //dataTable.columnDefs.
        });

        var $attraction_channel_table = $('#attraction-channel-table');

        $attraction_channel_table.on('click', 'tr', function (e) {
            if (!$(this).parent('thead').length && !$(this).find('.dataTables_empty').length && !hasTarget($(e.target), '.user_open_disable') && !dropDownOpened) {
                var id = $(this).data('id');
                openAttractionChannelForm(id);
            }
        });

        $attraction_channel_table.on('click', '.remove', function (e) {
            e.stopPropagation();
            if (confirm('Вы действительно хотите удалить ?')) {
                var $tr = $(this).closest('tr');
                var id = $tr.data('id');
                $.post('/attraction-channel/delete', {id: id, _csrf: _csrf}, function (response) {
    ...
    $('#open-new-attraction-channel-from').on('click', function (e) {
        openNewAttractionChannelForm();
    });
});
