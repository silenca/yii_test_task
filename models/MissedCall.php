 ...
 */
class MissedCall extends \yii\db\ActiveRecord
{
    /**
     * @inheritdoc
     */
    ...
    public function add($call_id, $manager_id) {
        $this->manager_id = $manager_id;
        $this->call_id = $call_id;
        return $this->save();
    }
    
    public static function remove($call_id, $manager_id) {
        $missed_call = self::find()->where(['call_id' => $call_id])->andWhere(['manager_id' => $manager_id])->one();
        if ($missed_call) {
            return $missed_call->delete();
        }
        return false;
    }
}
